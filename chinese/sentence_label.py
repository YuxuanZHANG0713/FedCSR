...
def generate_txt(data_path, label_txt_path):
    ...
    sentence_label_txt = open(label_txt_path + 'sentence_labels.txt', 'a')
    ...
    for p_dir in person_paths:
        ...
        for name in all_files:
            ...
            idx = int(pre.split('-')[1])
            if ext == '.mp4':
                if (p_dir=='hs' and idx <=1000) or (p_dir=='lf' and idx <=1000) or (p_dir=='wt' and idx <=1000) or (p_dir=='xp' and idx <=1000):
                    sentence_label = get_label(p_dir, pre)
...
def get_full_label(person_path, file_name, fps=30):
    ...
    # if (len(stamp_start) - len(text_seq)) != 2 or (len(stamp_end) - len(text_seq)) != 2 or len(
    #         text_seq) != num_intervals:
    #     print('TextGrid Error!')
    #     return None
