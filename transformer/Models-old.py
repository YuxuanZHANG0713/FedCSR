...
import sys
sys.path.append('C:\\Users\\dell\\Desktop\\SRIBD\\CuedSpeech\\FL\\Fed4CS-v2')
import torch
...
class CuedSpeechTransformer(nn.Module):
    ...
    def forward(self, src_seq, src_length, code_book):
        ...
        return output_ling, output_post, output_lengths
...
class LingLSTM(nn.Module):
    def __init__(self, n_trg_vocab = 43, 
        ...
        self.lstm = torch.nn.LSTM(d_words, hidden_size, num_layers = num_layers, batch_first = True)
        self.linear = torch.nn.Linear(hidden_size, n_trg_vocab)

    def forward(self, x):
        v_emb = self.code_emb(x)
        out, _ = self.lstm(v_emb)
        out = self.linear(out)
