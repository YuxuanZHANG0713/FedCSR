...
def get_target_mask(trg_seq, input_lengths):
    batchsize = trg_seq.size(0)
    non_pad_mask = trg_seq.new_ones(trg_seq.size())  # N x T
...
def apply_lm(parentBeam, childBeam, lm):
    ...
    """
    # print(parentBeam.labeling, childBeam.labeling)
    ...
    if not (childBeam.lmApplied):
        ...
        if parentBeam.lmState == None:
            initStateBatch = None
            inputBatch = torch.tensor(42).reshape(1,1)
            inputBatch = inputBatch.to(device)
        else:
            initStateBatch = parentBeam.lmState
            inputBatch = torch.tensor(parentBeam.labeling[-1]).reshape(1,1)
            inputBatch = inputBatch.to(device)
        ...
        with torch.no_grad():
            ...
            outputBatch = F.log_softmax(outputBatch, dim=-1)
            
        logProbs = outputBatch.squeeze()
        logProb = logProbs[childBeam.labeling[-1]]
...
def ctc_search_decode(outputBatch, inputLenBatch, beamSearchParams, spaceIx, eosIx, lm, blank=0):
    ...
    for n in range(len(outLogProbs)):
        ...
        for t in range(maxT):
            ...
            #considering only the characters with probability above a certain threshold to speeden up the algorithm
            ...
            for labeling in bestLabelings:
                ...
                for c in prunedChars:
                    ...
                    if lm is not None:
                        ...
                        apply_lm(curr.entries[labeling], curr.entries[newLabeling], lm)
